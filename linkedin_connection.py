...
def get_personal_info(driver, i, my_counter):
	...
	try:
		...
		with csvLock:
			...
			spamwriter.writerow([str(my_counter.value), int(connection_txt[:3]), result_name.text, 'NO INFO'])
	...
	with csvLock:
		...
		for j in range(0, min(len(result_text), len(result_text1))):
			spamwriter.writerow([str(my_counter.value), int(connection_txt[:3]), result_name.text, result_text[j].text, result_text1[j].text])
		my_counter.value += 1.0
